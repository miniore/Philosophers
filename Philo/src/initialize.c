...
static void	ft_assign_forks(t_args *args)
{
	...
	{
		if (i == 0)
			args->phi[i].l_fork = &args->phi[args->philo_num - 1].fork;
		else
			args->phi[i].l_fork = &args->phi[i - 1].fork;
		i++;
	}
}
...
int	philo_start(t_args *args)
{
	...
	i = 0;
	args->phi = (t_philo *)ft_calloc(args->philo_num, sizeof(t_philo));
	if (!args->phi)
	...
	while (i < args->philo_num)
	{
		args->phi[i].id = i + 1;
		args->phi[i].eating = 0;
		args->phi[i].num_eat = 0;
		args->phi[i].args = args;
		pthread_mutex_init(&args->phi[i].fork, NULL);
		pthread_mutex_init(&args->phi[i].lock, NULL);
		i++;
	}
